...
model.compile(optimizer=optimizers.Adam(0.0001), loss=losses.newGJL, metrics=[metrics.mdice])
savemodel=ModelCheckpoint('allinone.h5', monitor='val_mdice', verbose=1, save_best_only=True, save_weights_only=False, mode='max', period=1)
...
unlabeled_indxs=np.array(unlabeled_indxs)
x_train__,y_train__=update_with_new_pseudo(model,x_train,y_train,unlabeled_indxs,listaT1,listaFLAIR)
...
result=model.fit_generator(data_augmentation.DA_2Ddegradation(x_train__,y_train__), #model.fit(x_train, y_train,batch_size=1
                ...
                callbacks=[savemodel])
model.save_weights('step_allinone_'+str(step)+'.h5')
