...
out_filepath= lambda x: 'weights/data_gen_iqda_2it_isbi_TSNE3_bottleneckRegulirized_'+regularized_loss+'__'+str(loss_weights[0])+'_'+str(loss_weights[1])+'__Kclosest_'+"%02d" % (x)+'.h5'
...
if(load_precomputed_features):
    ...
    rank_distance=np.load('rank_distance_isbi_tsne3_regularized.npy')
else:
    ...
    np.save('rank_distance_isbi_tsne3_regularized.npy',rank_distance)
...
while(unlabeled_num>increment_new_data):
    ...
    if( resume and step==resume_after_adding_pseudo_of_step):
        ...
        update_data_folder(model,new_pos_in_features,listaT1,listaFLAIR,datafolder=datafolder,regularized=True)
    train_files_bytiles=[]
    for i in range(27):
