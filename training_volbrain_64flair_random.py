...
out_filepath= lambda x: 'weights/flair64_volbrain_TSNE3_bottleneckRegulirized_'+regularized_loss+'__'+str(loss_weights[0])+'_'+str(loss_weights[1])+'__random_'+"%02d" % (x)+'.h5'
...
while(unlabeled_num>increment_new_data):
    ...
    if( resume and step==resume_after_adding_pseudo_of_step):
        ...
        pseudolabeled_indxs= pseudolabeled_indxs+ new_pseudo
        unlabeled_indxs =  [x for x in unlabeled_indxs if x not in pseudolabeled_indxs]
        update_data_folder_flair(model,new_pseudo,listaFLAIR,listaMASK,datafolder=datafolder,regularized=regularized)
        unlabeled_num=len(unlabeled_indxs)
    train_files_bytiles=[]
    for i in range(27):
