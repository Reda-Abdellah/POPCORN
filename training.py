...
while(unlabeled_num>increment_new_data):
    ...
    if(pseudolabeled_num>0):
        ...
        #rank_distance= pca_rank(bottleneck_features_unlabeled, np.concatenate((bottleneck_features_labeled,bottleneck_features_pseudolabeled),axis=0),n_components=2)
        rank_distance= tsne_rank(bottleneck_features_unlabeled, np.concatenate((bottleneck_features_labeled,bottleneck_features_pseudolabeled),axis=0),n_components=3)
    else:
    ...
    result=model.fit_generator(data_augmentation.DA_2Ddegradation(x_train__,y_train__),#data_augmentation.MixUp_Generator(x_train,y_train,0.3),DA_2Ddegradation, #model.fit(x_train, y_train,batch_size=1
                    ...
                    validation_data=(x_val, y_val),
                    callbacks=[savemodel])
    model.save_weights('step_IQDA_TSNE3'+str(step)+'.h5')
